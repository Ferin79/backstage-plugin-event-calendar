...
import { eventCalendarBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [eventCalendarBackendPlugin],
    });

    await request(server).get('/api/event-calendar-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/event-calendar-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/event-calendar-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/event-calendar-backend/todos/${createdTodoItem.id}`)
